...
def mock(L_box, N_grid, axionFrac, redshift, start_time, dirName_plot, mpi_on=False,comm=None, rank=None):
    """
    ...
    """
    if not mpi_on or rank == 0: print(f"Starting at {start_time}")

    #################################################################################### FILE NAMES
    dirName = f"forestsNBK{axionFrac}{int(L_box.value)}" 
    file_param = f"{dirName}/my_params.csv" #f"{dirName}/my_params.csv"
    file_mocks = {"dens": f"{dirName}/mocks.npy", "vel_x": f"{dirName}/vel_x.npy", "vel_y": f"{dirName}/vel_y.npy", "vel_z": f"{dirName}/vel_z.npy"}

    #################################################################################### PARAMETERS OF SIMULATION
    
    # Cosmo Parameters
    cosmo = cosmology.Planck15
    H_0 = 100. * cosmo.h * u.km /u.s /u.Mpc                                                         # in km * s^-1 * Mpc^-1 * h
    H_z = H_0 * np.sqrt(cosmo.Om0 * (1+redshift)**3 + cosmo.Ode0)                                   # in km * s^-1 * Mpc^-1 * h (Bird_2023 p.4)
    h_z = H_z / (100. * u.km /u.s /u.Mpc)                                                           # unitless
    rho_crit = (3. * H_z**2. / (8. * np.pi * c.G)).to(u.M_sun / (u.kpc)**3)                         # in M_sun/h / (kpc/h)^3 
    ...
    if not mpi_on or rank == 0: print(f"L_box: {L_box}, N_grid: {N_grid}")
    ...
    mean_density = cosmo.Ob0 * rho_crit # Mean matter density                                       # in M_sun/h / (kpc/h)^3
    total_mass = mean_density * (L_box.to(u.kpc))**3                                                # in M_sun/h ???????????????????????
    #if not mpi_on or rank == 0: print(f"\nmean density: {mean_density:.2f}\n") 
    params_axionCAMB = my.getParams(f"Archive/PJC_{axionFrac}_params.ini", foresting=True)
    if not mpi_on or rank == 0: print("my H_0: ", H_0, "axiCAMB's: ", params_axionCAMB["hubble"])
    ...
    with open(file_param, 'w', newline='') as p:
        ...
        writer.writerow(['mean_density', (mean_density).value])                                     # in M_sun/h / (kpc/h)^3
        writer.writerow(['b1', b1])
        writer.writerow(['axfrac', axfrac])
        writer.writerow(['m_ax', m_ax])

    #################################################################################### GENERATE CATALOG
    if not mpi_on or rank == 0: print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: done importing, generating catalog")
    ...
    #################################################################################### CREATE DENSITY MESH
    if not mpi_on or rank == 0: print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: done with catalog, generating mesh")
    compensated = False
    ...
    if not mpi_on or rank == 0: print(f"1+delta: mean {one_plus_delta.mean():.2f} min {one_plus_delta.min():.5f}, max {one_plus_delta.max():.2f}")
    print("rank: ", rank, "mesh: ", one_plus_delta.shape)

    if compensated:
        one_plus_delta_temp = (one_plus_delta - np.min([0., np.min(one_plus_delta.value)]))
    ...
    vel_x_mesh = mock.to_mesh(position='Position', value='Velocity_x', resampler='tsc', compensated=False, dtype='f4')
    vel_y_mesh = mock.to_mesh(position='Position', value='Velocity_y', resampler='tsc', compensated=False, dtype='f4')
    vel_z_mesh = mock.to_mesh(position='Position', value='Velocity_z', resampler='tsc', compensated=False, dtype='f4')
    vel_x = vel_x_mesh.preview(Nmesh=N_grid, root=0) * u.km / u.s  # .compute().value * u.km / u.s # compute() turns to realF<ield, .value to np.array
    vel_y = vel_y_mesh.preview(Nmesh=N_grid, root=0) * u.km / u.s  # .compute().value * u.km / u.s
    vel_z = vel_z_mesh.preview(Nmesh=N_grid, root=0) * u.km / u.s  # .compute().value * u.km / u.s

    #vel_field = np.stack([vel_x, vel_y, vel_z], axis=0) # shape: (3, N_grid, N_grid, N_grid)

    if not mpi_on or rank == 0: print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: saving")
    if mpi_on:
        # Only rank 0 saves
        if rank == 0:
            # Verify final shape matches expectations
            if den_field.shape != (N_grid, N_grid, N_grid):
                print(f"Warning: Gathered shape {den_field.shape} differs from expected {(N_grid, N_grid, N_grid)}")
...
def foresting(mpi_on, mocked, axionFrac, L_box, N_los, HI, start_time, dirName_plot, extraEnding="", comm=None, rank=None, size=None, plotose=False):
    """
    ...
    if mocked:
        ...
        file_mocks = {"dens": f"{dirName}/mock.npy", "vel_x": f"{dirName}/vel_x.npy", "vel_y": f"{dirName}/vel_y.npy", "vel_z": f"{dirName}/vel_z.npy"}

    file_param = f"{dirName}/my_params.csv" #f"{dirName}/my_params.csv"
    files = {"specs": [f"{dirName}/download/lambda_range.csv", f"{dirName}/download/forest_x.csv", f"{dirName}/download/forest_y.csv", f"{dirName}/download/forest_z.csv"],
            "dens": [f"{dirName}/download/x.csv", f"{dirName}/download/den_x.csv", f"{dirName}/download/den_y.csv", f"{dirName}/download/den_z.csv"]}

    #################################################################################### GET MOCKING PARAMS
    if not mpi_on or rank == 0: print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: done importing, reading files")
    ...
    if "nbar" in params.keys(): nbar = params["nbar"]
    else: nbar = 3*(N_grid/L_box.value)**3

    if "H_z" in params.keys(): H_z = params["H_z"] * u.km / u.s / u.Mpc                            # in km/s * h/Mpc
    else:
        H_0 = 100. * FW_h * u.km /u.s /u.Mpc                                                         # in km * s^-1 * Mpc^-1 * h
        H_z = H_0 * np.sqrt(FW_OmM * (1+redshift)**3 + FW_OmL)                                   # in km * s^-1 * Mpc^-1 * h (Bird_2023 p.4)

    if "mean_density" in params.keys(): mean_density = params["mean_density"] * u.M_sun / (u.kpc)**3
    else: 
        rho_crit = (3. * H_z**2. / (8. * np.pi * c.G)).to(u.M_sun / (u.kpc)**3)                         # in M_sun/h / (kpc/h)^3 
        mean_density = FW_OmM * rho_crit

    if "b1" in params.keys(): b1 = params["b1"]             
    else: b1 = 2
    ...
    delta_z = (H_z * L_box/c.c.to(u.km/u.s)).value # delta_z=0.0296 for L_box=11,1Mpc/h and z=3                         # cf Croft_1997 p.7
    line_obs = my.get_lambda_from_d(L_box, H_z, line)      # line * (delta_z + 1)                   # Angstroms
    lambda_int = line_obs-line                                                                      # Angstroms
    lambda_margin = 0. * u.Angstrom  # 5                                                            # Angstroms
    ...
    res = (obs_res * line / c.c).to(u.Angstrom)       # ∆v = c∆λ/λ = 69km/s                         # Angstroms/pixel   # cf Palanque-Delabrouille_2013 p6

    N_spectrum = int(((lambda_int+lambda_margin*2)/res).value)

    N_spectrum = np.max((N_spectrum - N_spectrum%N_grid + N_grid, 3*N_grid)) # + N_spectrum//N_grid
    ...
    """
    if not mpi_on or rank == 0: print(f"L_box: {L_box}, N_grid: {N_grid}, N_los: {N_los}, N_spectrum: {N_spectrum}, delta_z: {delta_z:.4f}, lambda_int: {lambda_int:2f}")
    ...
    Temp_0 = 1e4 *u.K * (1 + redshift) # Temperature at mean density McQuinn_2016 p.16              # in K
    # optical depth approximation Viel_2004 p5
    tau_0 = 0.305; # optical depth at mean density depends on redshift, baryon density,
                # temperature at mean density, Hubble constant and photoionisation rate
    tau_s = 1.7*gamma; # slope of optical depth-density relation 2.7-0.7 * gamma (cf Viel_2004 p5)
    ...
    if not mpi_on or rank == 0:
        ...
        with open(file_param,'w') as p:
            ...
            writer.writerow(['lambda_end', (line_obs + lambda_margin).value])                           # Angstrom
    ...
    if not mpi_on or rank == 0: print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: computing spectra")

    my_dir, start, end = my.organizeAbsorption(mpi_on, size, rank, N_los)

    print("rank: ", rank, "my dir: ", my_dir, "start: ", start, "end: ", end)
    if mpi_on:
        lambda_range, spectra = my.computeAbsorption(my_dir, start, end,
                        N_grid, N_los, N_spectrum, L_box, res,
                        file_mocks, HI, mocked,
                        redshift, H_z, line, line_obs, lambda_margin, einstein_Acoeff,
                        Temp_0, gamma, tau_0, tau_s,
                        mpi_on, rank, dirName_out=f"{dirName}_MPI")

        my.write_spectra_file(
        spectra, lambda_range,
        files,
        mpi_on, comm, size, rank,
        N_los, N_spectrum, end, start,)
        
        my.write_den_file(my_dir, N_los, N_grid, L_box, H_z, line, file_mocks, files, mpi_on, rank)   
        
    else:
        final_spectra = []
        for i in my_dir:
            lambda_range, spectra = my.computeAbsorption(i, start, end,
                            N_grid, N_los, N_spectrum, L_box, res,
                            file_mocks, HI, mocked,
                            redshift, H_z, line, line_obs, lambda_margin, einstein_Acoeff,
                            Temp_0, gamma, tau_0, tau_s,
                            mpi_on, rank, dirName_out=dirName)
            my.write_den_file(i, N_los, N_grid, L_box, H_z, line, file_mocks, files, mpi_on, rank)
            final_spectra.append(spectra)
        my.write_spectra_file(
            np.asarray(final_spectra), lambda_range,
            files,
            mpi_on, comm, size, rank,
            N_los, N_spectrum,
            end, start,)
    ...
    if (not mpi_on or rank == 0) and (plotose):
        #################################################################################### TESTING
        print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: testing spectra")
        my.testSpectra(file_param, files, titlePlots, dirName_plot, foresting=True, tot=True)
        
        print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: testing slices")
        my.plotMock(L_box, N_grid, files, mocked, Temp_0, gamma, dirName_plot, titlePlots, savefig=True)




def analyze(axionFrac, boxSize, mocked, J, Q, order, log_binning, fit, savefigs, start_time, dirName_plot, extraEnding="", dirName_in=None, dirName_out=None, csv_on=True, filter_on=False, window_on=False):
    """
    ...
    files = {"specs": [f"{dirName_in}/lambda_range", f"{dirName_in}/forest_x", f"{dirName_in}/forest_y", f"{dirName_in}/forest_z"],
            "dens": [f"{dirName_in}/x", f"{dirName_in}/den_x", f"{dirName_in}/den_y", f"{dirName_in}/den_z"]}
    ...
    #################################################################################### GETTING PARAMS
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: done importing, reading files")
    ...
    delta_lambda = params["lambda_end"]-params["lambda_start"]-params["lambda_margin"]*2
    ...
    k_min = 2 * np.pi/(L_box) # * H_z)                              # cf. Arinyio-i-Prats p.12
    ...
    k_max = nbins*np.pi/(L_box) #*H_z)                              # Nyquist frequency cf. Arinyio-i-Prats p.12
                                                                    # and https://nbodykit.readthedocs.io/en/latest/cookbook/interlacing.html
    ...
    if not filter_on: sigma_filter_ind_spec_forests = None
    else: sigma_filter_ind_spec_forests = nbins/N_spectrum
    ...
    if not window_on: sigma_Window_PS = None
    else: sigma_Window_PS = (2*np.pi/my.convert_k_to_velocity_space(2*np.pi*nbins/L_box, redshift, H_z)) / np.sqrt(8*np.log(2)) # FWHM-->sigma
    ...
    print("sigma spec: ", sigma_filter_ind_spec_forests, "sigma den: ", sigma_filter_ind_spec_den)
    ...
    #################################################################################### GETTING DATA
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: getting data")
    forests, lambda_range, densities, x_for_den = my.getData(files, N_spectrum, N_grid, N_los, csv_on) # forests

    
    # Undersampling
    undersampling_mask = len(lambda_range)//(len(x_for_den)) #*2)              # divided by 2 because k sampling for real FFT will divide by 2
    forests = forests[:, :, :, ::undersampling_mask]
    lambda_range = lambda_range[::undersampling_mask]
    if len(lambda_range)==(len(x_for_den)+1): #* 2) + 1:                       # divided by 2 because k sampling for real FFT will divide by 2
        forests = forests[:, :, :, :-1]
        lambda_range = lambda_range[:-1]
    elif len(lambda_range)!=(len(x_for_den)): #*2):                            # divided by 2 because k sampling for real FFT will divide by 2
        raise ValueError("len(lambda_range)!=(len(x_for_den) * 2)", len(lambda_range), (len(x_for_den) * 2))
    N_spectrum = len(lambda_range)
    print(N_spectrum)
    nsamples = int(N_spectrum/(2**J))    
    

    my.plotForestsDens(lambda_range, forests, x_for_den, densities, line, line_obs, # [:,:30,:30,:] #----------WARNING! :30 slice because the NBK was killed before finish on 3007
                        titlePlots["mat/flux"], dirName_plot, filename=f"spectra", tot=True, losToPlot=1)

    if sigma_filter_ind_spec_forests!=None:
        forests = my.gaussian_filter1d(forests, sigma=sigma_filter_ind_spec_forests, axis=-1, truncate=1.)      # shape (3, N_los, N_los, N_spectrum/2)

    if sigma_filter_ind_spec_den!=None:
        densities = my.gaussian_filter1d(densities, sigma=sigma_filter_ind_spec_den, axis=-1, truncate=1.)      # shape (3, N_los, N_los, N_spectrum/2)

    my.plotForestsDens(lambda_range, forests, x_for_den, densities, line, line_obs, # [:,:30,:30,:] #----------WARNING! :30 slice because the NBK was killed beofre finish on 3007
                        titlePlots["mat/flux"], dirName_plot, filename=f"spectra_smooth", tot=True, losToPlot=losToPlot)

    print(f"############################----- DENSITY -----##############################")
    #################################################################################### COMPUTING PDF
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: computing PDF")
    one_plus_delta = densities/np.mean(densities)
    my.plotPDF(one_plus_delta, nbinsPDF=1000, range =(0,2), x_label=r"Matter density $\varrho$", title=titlePlots["PS mat"], dirName_plot=dirName_plot, save_to=pdf_file, xlog=False, ylog=False, filename="densities3", savefig=True)
    my.plotPDF(one_plus_delta, nbinsPDF=1000, range =(-4,2), x_label=r"Perturbations $\delta$", title=titlePlots["PS mat"], dirName_plot=dirName_plot, save_to=pdf_file, xlog=True, ylog=False, filename="delta", savefig=True)
    ...
    #################################################################################### COMPUTING PDF
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: computing PDF")
    my.plotPDF(forests, nbinsPDF=1000, range =(0,100), x_label=r"Flux $F$", title=titlePlots["PS mat"], dirName_plot=dirName_plot, save_to=pdf_file, xlog=False, ylog=True, filename="flux", savefig=True)
    my.plotPDF(forests/np.mean(forests), nbinsPDF=1000, range =(-9,0), x_label=r"Flux perturbations $\delta F$", title=titlePlots["PS mat"], dirName_plot=dirName_plot, save_to=pdf_file, xlog=True, ylog=True, filename="deltaF", savefig=True)

    #################################################################################### COMPUTING POWER SPECTRA
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: computing 1D power spectrum")
    kz, kbins_edges, kbins_centers = my.sample_k(k_min, k_max, nbins, N_spectrum, log_binning) # Precompute bins once
    kz_vel = my.convert_k_to_velocity_space(kz, redshift=redshift, H_z=H_z)
    kbins_edges_vel = my.convert_k_to_velocity_space(kbins_edges, redshift=redshift, H_z=H_z)
    kbins_centers_vel = my.convert_k_to_velocity_space(kbins_centers, redshift=redshift, H_z=H_z)
    ...
    Pk_1D_vel = my.compute_all_1d_power_spectra(forests, kz_vel, kbins_edges_vel, L_box, N_grid, R,
                                                sigma_filter_spec=sigma_filter_ind_spec_forests, sigma_filter_PS=sigma_Window_PS)
    ...
    Pk_3D_vel = my.oneD_to_threeD_powSpectr(Pk_1D_vel, kbins_centers_vel, sigma_Window_PS3D) # shape (3, N_los, N_los, N_spectrum/2)
                # Pk_3D.copy() # 1/my.convert_k_to_velocity_space(1/Pk_3D)
    if Pk_fit != None: Pk_3D_fit = my.oneD_to_threeD_powSpectr(Pk_fit, kbins_centers_vel, sigma_Window_PS3D)
    else: Pk_3D_fit = None

    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: getting means&medians")
    #Pk_1D_tot = my.converge(kbins_centers, Pk_1D, dir=3, shape=(0,1)) #remember: first element will be cut off
    Pk_1D_tot_vel = my.converge(kbins_centers_vel, Pk_1D_vel, Pk_fit, dir=3, shape=(0,1))
    #Pk_3D_tot = my.converge(kbins_centers, Pk_3D, dir=3, shape=(0,1))
    Pk_3D_tot_vel = my.converge(kbins_centers_vel, Pk_3D_vel, Pk_3D_fit, dir=3, shape=(0,1))

    #################################################################################### COMPUTING WAVELET SCATTERING TRANSFORMS
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: computing 1D WST")
    WST_coeffs = my.compute_all_1d_WST(forests, J, Q, order, sigma_filter_ind_spec_forests, sigma_Window_WST, out_type='list')
            # shape WST: (ncoeffs, ["coef",             3*N_los*N_los, nsamples)         nsamples=N_spectrum/2^J
            #                       "n", "j"]           1 or 2)
    ncoeffs = len(WST_coeffs)
    if nsamples != WST_coeffs[0]['coef'].shape[1]: raise ValueError(f"nsamples != N_spectrum/2^J:         {nsamples}!={WST_coeffs[0]['coef'].shape[1]}")
    print(nsamples)
    ...
    _,_,k_samples = my.sample_k(k_min, k_max, nbins=nsamples, N_spectrum = N_spectrum, log_binning=log_binning)
    k_samples_vel = my.convert_k_to_velocity_space(k_samples, redshift=redshift, H_z=H_z)

    WST_tot = my.summerize_WST(WST_coeffs, J, order)


    #################################################################################### WRITING RESULTS TO FILE
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: saving powSpectrum to file")

    np.save(k_PS_file, kbins_centers_vel) # shape (N_spectrum/2)
    np.save(k_WST_file, WST_tot["x"]) # shape (n_coeffs)
    np.save(powSpectr_file, Pk_1D_vel.reshape(3*N_los*N_los, -1).T) # shape (N_spectrum/2, n_los)
    ...
    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: plotting power spectrum 1D")
    # print("k shape powSpectr: ", kbins_centers_vel.shape, "\nPk1D shape powSpectr: ", Pk_1D_vel.shape, "\nPk1Dtot shape powSpectr: ", Pk_1D_tot_vel.shape)
    
    
    my.plotPowSpectr(kbins_centers_vel, Pk_1D_vel, Pk_1D_tot_vel, #kbins_centers_vel[:100], Pk_1D_vel[:,:,:,:100], Pk_1D_tot_vel[:,:100]
                    colors=colors, x_label=r"$k\ [\mathrm{s/km}]$", y_label= r"$P_{flux}(k)\ [\mathrm{km/s}]$", losToPlot=losToPlot,
                    filename="1D_vel_FLUX", dirName_plot=dirName_plot, title=titlePlots["PS flux"], savefig=savefigs[0],
                    summary=True, fit=fit, dir=3, oneD=True)

    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: plotting power spectrum 3D")
    Delta2 = Pk_3D_vel * kbins_centers_vel**3 /(2*np.pi**2) # L_box**3 / (2*np.pi)**3 * 4*np.pi * kbins_centers_vel**3 * Pk_3D_vel
    Delta2_tot = Pk_3D_tot_vel * kbins_centers_vel**3 /(2*np.pi**2) # L_box**3 / (2*np.pi)**3 * 4*np.pi * kbins_centers_vel**3 * Pk_3D_tot_vel
    
    my.plotPowSpectr(kbins_centers_vel, Delta2, Delta2_tot, 
                    colors=colors, x_label = r"$k\ [\mathrm{s/km}]$", y_label = r"$\Delta^2(k)\ [-]$", losToPlot=losToPlot,
                    filename="3D_vel_FLUX", dirName_plot=dirName_plot, title=titlePlots["PS flux"], savefig=savefigs[1],
                    summary=True, fit=fit, dir=3, oneD=False)



    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: plotting WST general")
    my.plotWS_all(k_samples_vel, WST_coeffs, J, order,
                colors=colors, xlabels=None, ylabels=None, losToPlot=losToPlot**2,
                filename="wst", dirName_plot=dirName_plot, title=titlePlots["WST flux"], savefig=savefigs[2])

    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: plotting WST summarized")
    my.plotWS_summary(WST_tot, J, order,
            colors=colors, xlabels=None, ylabels=None, losToPlot=losToPlot**2,
            filename="wst_summary", dirName_plot=dirName_plot, title=titlePlots["WST flux"], savefig=savefigs[3],
            stat=True) ########### True: means, False: medians

    print(f"----------------- sec {(Timestamp.now()-start_time).total_seconds():.2f}: DONE")
